...
int main(int argc, char **argv) {
  ...
  // A throwaway /etc/pam.d, so a slot can be driven against a freshly built
  // module without editing the one the real lock screen uses.
  const char *confdir = argc > 4 ? argv[4] : NULL;
