...
The installer installs the GNOME Shell extension package and tries to enable lock screen face unlock for the current GNOME user. If you installed packages manually or automatic enablement was not possible, log out and back in once (so GNOME Shell scans the new extension) and then run:
...
> Running `gnome-extensions enable` before logging back in will return `Extension "gaze@gundulabs.com" does not exist` — Shell only rescans extension directories at session start.
...
For GNOME lock screen face unlock after manual package installation, log out and back in once, then from your GNOME session run `gnome-extensions enable gaze@gundulabs.com` and `gsettings set org.gnome.shell.extensions.gaze enable-face-authentication true`.
