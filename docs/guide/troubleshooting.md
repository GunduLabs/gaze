...
If `gnome-extensions enable` reports `Extension "gaze@gundulabs.com" does not exist`, GNOME Shell has not picked up the newly installed extension yet. Log out and back in (or reboot), then re-run the command. On Wayland this is the only way — Shell does not rescan extensions in a running session. The one-line installer works around this by writing the equivalent dconf keys directly, which take effect on the next login without needing `gnome-extensions enable` to succeed.
