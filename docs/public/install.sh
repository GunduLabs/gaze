...
enable_gnome_extension() {
    if [ "$(id -u)" -eq 0 ]; then
        ...
        echo "For GNOME lock screen face unlock, log out and back in once, then run as your desktop user:"
    ...
    if ! is_gnome_session; then
        ...
        echo "For GNOME lock screen face unlock, log out and back in once, then from your GNOME session:"
        echo "  gnome-extensions enable gaze@gundulabs.com"
        echo "  gsettings set org.gnome.shell.extensions.gaze enable-face-authentication true"
        echo "GDM loads the extension by default. Login face auth requires the docs command: ${GNOME_DOCS_URL}"
        return 0
    ...
    if command -v gnome-extensions >/dev/null 2>&1 && gnome-extensions enable "$EXT_ID" >/dev/null 2>&1 && _gsettings_enable_face_auth; then
        ...
        echo "Registered GNOME lock screen face unlock via dconf — log out and back in once to activate it."
        echo "Note: running 'gnome-extensions enable $EXT_ID' before that logout will report \"Extension does not exist\"; the dconf entry just written makes that step unnecessary."
    else
        echo "Could not enable the GNOME extension automatically."
        echo "Log out and back in once, then from your GNOME session run:"
        echo "  gnome-extensions enable gaze@gundulabs.com"
        echo "  gsettings set org.gnome.shell.extensions.gaze enable-face-authentication true"
    fi
