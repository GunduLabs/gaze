...
greetd, and a plain TTY do not. At a greeter with no session the daemon captures
the seat's V4L2 device directly instead, so `primary` still works; elsewhere
without a session (a plain TTY, for instance) point `rgb` at the camera directly,
which uses `v4l2src` and needs no session:
...
| `all` | Every face authentication waits, `sudo` and polkit prompts included. |

`screen_lock` is the default, so a delay you set applies to lockers only unless you
ask for `all`. To be explicit:
