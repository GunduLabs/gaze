...
| Interfaces | CLI + GTK GUI + GNOME ext + Hyprland + COSMIC | CLI only | CLI only | GUI (Tauri) + CLI |
| Desktop integration | GNOME lock screen, Hyprland/hyprlock, COSMIC lock + login | PAM only | PAM only | PAM + polkit |
