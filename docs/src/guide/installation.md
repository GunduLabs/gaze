...
### Enable face unlock for COSMIC

On COSMIC (Pop!_OS and the COSMIC spins), install the `gaze-cosmic` package (auto-installed by the one-line installer when COSMIC or `cosmic-greeter` is detected). It wires `/etc/pam.d/cosmic-greeter`, which covers both the COSMIC lock screen and the cosmic-greeter login screen; the GNOME Shell extension is skipped there. See the [COSMIC guide](/guide/cosmic).
