...
async fn verify_within(username: &str, service: Option<&str>, budget: Duration) -> Verdict {
    verify_until(budget, service_retries_transient_give_up(service), || {
        authenticate_biometric_with_status(username, service)
    })
    .await
}
...
unsafe fn do_authenticate(pamh: PamHandle) -> c_int {
    ...
    let matched = rt.block_on(async {
        ...
        match verify_within(&username, service.as_deref(), budget).await {
            Verdict::Reached(AuthOutcome::Match, _) => Ok(loaded_auth),
