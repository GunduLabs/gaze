...
fn parse_profile(path: &Path) -> ProcessedProfile {
    ...
    let (file_vid, file_pid) = parse_filename_ids(file_stem).unwrap_or_else(|| {
        panic!("profile file must be named vvvv-pppp.toml: {}", path.display())
    });
    ...
    ProcessedProfile {
        ...
        name: p.device.name,
        source: p.device.source.unwrap_or_else(|| "gaze-core profile".into()),
        on,
        off,
    }
}
...
fn render(profiles: &[ProcessedProfile]) -> String {
    ...
    writeln!(out, "// Do not edit directly; add ir-profiles/*.toml instead.\n").unwrap();
